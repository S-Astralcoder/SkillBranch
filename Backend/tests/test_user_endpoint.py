...
def test_jwt_token_generation_after_signup():
    response = client.post("/user/login", data={"username": test_user["email"], "password" : test_user["password"]})
...
@pytest.mark.parametrize(("invalid_user"), [
    ...
    {"username" : "blaze" * 100, "email" : "blaze@gmail.com", "password" : "blazebuster1234"},
    {"username" : "blaze" * 100, "email" : "blaze@gmail.com" * 200, "password" : "blazebuster1234" * 200},  
])
...
def test_reject_issue_jwt_for_non_registered_user():
    response = client.post("/user/login", data={"username": "test@my.com", "password" : test_user["password"]})
    assert response.status_code == 404

def test_reject_issue_jwt_for_invalid_password_user():
    response = client.post("/user/login", data={"username": test_user["email"], "password" : "password123"})
    assert response.status_code == 401

def test_reject_non_email_in_username_field():
    response = client.post("/user/login", data={"username": test_user["username"], "password" : "password123"})
    print(response.json())
