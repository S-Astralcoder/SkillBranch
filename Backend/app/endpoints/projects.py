...
async def get_all_projects(
    ...
    if not get_skill_by_id(skill_id=skill_id, user_id=user.id, db_session=db_session):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Given Skill doesn't exist"
        )

    query = (
        select(Project)
        .join(Skill, Project.skill_id == Skill.id)
        .where(Skill.user_id == user.id, Skill.id == skill_id)
...
async def get_project(
    ...
    if not project:
        raise HTTPException(
            ...
            detail="The given project doesn't exists",
        )
    return project
...
async def create_project(
    ...
    skill = get_skill_by_id(
        skill_id=payload.skill_id, user_id=user.id, db_session=db_session
    )
    if not skill:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Given Skill doesn't exist"
    ...
    if get_project_by_name(
        ...
        raise HTTPException(
            ...
            detail="The given project name already exists",
        )
    project = Project(
        project_name=payload.project_name,
...
async def delete_project(
    ...
    if not skill:
        raise HTTPException(
            ...
            detail="I don't even know how this was triggered",
        )
    print(skill)
    print(project)
